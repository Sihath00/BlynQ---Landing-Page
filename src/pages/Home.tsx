...
const testimonials = [
  ...
  },
  {
    ...
    role: "Web Developer",
    content:
      "We believe technology should simplify lives, and BlynQ does just that—making vehicle management stress-free and smarter than ever before.",
    rating: 5,
  },
  {
    ...
    role: "Mobile App Developer",
    content:
      "We built BlynQ to eliminate the hassle of tracking services, managing appointments, and handling vehicle data. Now, it’s all in one seamless system.",
    rating: 5,
  },
];
...
export const Home = () => {
  ...
  return (
    ...
    >
      <h2 className="text-5xl font-extrabold text-blue-900 mb-4">Built for You, Designed by Us </h2>
      <p className="text-lg text-gray-700">Trusted by vehicle owners and service centers across Sri Lanka</p>
    </motion.div>

    <motion.div
      className="flex flex-nowrap gap-10 overflow-hidden"
      drag="x"
      dragConstraints={{ left: 0, right: 0 }}
      style={{ cursor: "grab" }}
      animate={{
      x: [0, -500, 0], // Adjust the values to control the swipe distance
      }}
      transition={{
      duration: 20, // Adjust the duration to control the swipe speed
      repeat: Infinity,
      ease: "linear", // Use "linear" for a constant swipe speed
      }}
    >
      {testimonials.map((testimonial, index) => (
      <motion.div
      key={index}
      className="relative bg-white p-8 rounded-3xl shadow-2xl hover:shadow-3xl transition-all duration-500 w-[400px] flex-shrink-0"
      >
